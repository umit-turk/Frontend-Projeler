...
import { Button, Input, Select, Space, Table, Tag, Form } from "antd";
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (updateAt: string, record: Record) => {
        const updateAtObj = new Date(updateAt);
        return (
          <>
            {updateAtObj.toLocaleDateString()}{" "}
            {updateAtObj.toLocaleTimeString("tr-TR", {
              hour: "2-digit",
              minute: "2-digit",
            })}
          </>
